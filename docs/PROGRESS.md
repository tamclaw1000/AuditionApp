...
## Notes
- **`APPLICATION-REQUIREMENTS.md` and `TECHNICAL-REQUIREMENTS.md` refactored (2026-06-01)** to reflect all decisions here, using the new terminology. The previously-parked concepts (per-piece recording-attempt limits, applicant private per-recording notes, offline mode + wifi-only offload, external-mic support, tech-support/contact-camp sections, future AI scan) are now folded into `APPLICATION-REQUIREMENTS.md`. The history-visibility conflict is reconciled there per item 16.
- **`ARCHITECTURE.md` left untouched** — its terminology rollout (Musicians→Applicant, Organizations→Camp/Camp Manager, Application entity→Audition, drop "Event") is sequenced as one pass with the item-17 data-model rework. ~105 old-term occurrences remain there + in the 3 diagram exports.
